...
use think\Request;
use \app\common\model\FeedbackInfo;
use \Exception;
use \think\exception\DbException;

class FeedbackInfoController extends Controller
{
    protected static $model;
    const PARAM_FLAG = 'param';

    public function __construct(App $app, FeedbackInfo $model)
    {
    ...
    public function index()
    {
        ...
        $data = self::$model
            ...
            ->toArray();
        $list = self::$model->with('personnel')->select()->toArray();
        var_dump($list);exit();
